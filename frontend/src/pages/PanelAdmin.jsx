...
function PanelAdmin() {
  ...
  const cargarDatos = async () => {
    const [resVendedores, resStats] = await Promise.all([
      ...
      {estadisticas && (
        <div style={styles.statsRow}>
          <div style={styles.statCard}>
            <span style={styles.statNum}>{estadisticas.total_vendedores}</span>
            <span style={styles.statLabel}>Total vendedores</span>
          </div>
          <div style={styles.statCard}>
            <span style={styles.statNum}>{estadisticas.vendedores_activos}</span>
            <span style={styles.statLabel}>Activos</span>
          </div>
          <div style={styles.statCard}>
            <span style={styles.statNum}>{estadisticas.vendedores_pendientes}</span>
        ...
        {vendedores.map((v) => (
          <div key={v.id} style={styles.fila}>
            <div style={styles.info}>
              ...
              {v.codigo_catalogo && (
                <span style={styles.codigo}>Código: {v.codigo_catalogo}</span>
              )}
              <span style={styles.limite}>Tableros: {v.limite_tableros}</span>
            </div>
            <div style={styles.acciones}>
              {v.estado === "pendiente" && (
                <button style={styles.btnAprobar} onClick={() => aprobar(v.id)}>
                  Aprobar
                </button>
              )}
              {v.estado === "activo" && (
                <button style={styles.btnDeshabilitar} onClick={() => deshabilitar(v.id)}>
                  Deshabilitar
                </button>
              )}
              <button
                style={styles.btnLimite}
                onClick={() => {
...
const estadoColor = (estado) => ({
  ...styles.estadoBadge,
  backgroundColor:
    estado === "activo"       ? "#d4edda" :
    estado === "pendiente"    ? "#fff3cd" : "#f8d7da",
  color:
    estado === "activo"       ? "#155724" :
    estado === "pendiente"    ? "#856404" : "#721c24",
});
...
const styles = {
  ...
  btnDeshabilitar:  { backgroundColor: "#dc3545", color: "#fff", border: "none", borderRadius: "8px", padding: "0.5rem 1rem", cursor: "pointer", fontSize: "0.9rem" },
