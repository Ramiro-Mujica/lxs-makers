...
// Vista pública: buscar catálogo por código y consultar seguimiento de pedido
...
function Inicio() {
  ...
  const buscarCatalogo = () => {
    if (codigoCatalogo.trim()) {
      window.location.href = `/catalogo/${codigoCatalogo.trim()}`;
    }
  };

  const consultarSeguimiento = () => {
    if (codigoSeguimiento.trim()) {
      window.location.href = `/seguimiento/${codigoSeguimiento.trim()}`;
    }
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        <input
          style={styles.input}
          type="text"
          placeholder="Ingresá el código de la tienda"
          value={codigoCatalogo}
          onChange={(e) => setCodigoCatalogo(e.target.value)}
        />
        <button style={styles.boton} onClick={buscarCatalogo}>
          Buscar tienda
        </button>
      </div>
      ...
      <div style={styles.card}>
        ...
        <input
          style={styles.input}
          type="text"
          placeholder="Ingresá tu código de seguimiento (8 caracteres)"
          maxLength={8}
          value={codigoSeguimiento}
          onChange={(e) => setCodigoSeguimiento(e.target.value)}
        />
        <button style={styles.boton} onClick={consultarSeguimiento}>
          Consultar pedido
        </button>
      </div>
    </div>
