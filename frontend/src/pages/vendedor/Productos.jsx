    ...
    const extraerError = (err, mensajeDefault) => {
    const data = err.response?.data
    if (typeof data?.detail === 'string') return data.detail
    if (Array.isArray(data?.detail)) return data.detail[0]?.msg || mensajeDefault
    ...
    const handleSubmit = async (e) => {
        ...
        setError(extraerError(err, 'Error al crear producto.'))
        }
    }
    ...
    const handleEditar = async (e) => {
        ...
        setError(extraerError(err, 'Error al editar producto.'))
        } finally {
        setGuardando(false)
        }
    }
