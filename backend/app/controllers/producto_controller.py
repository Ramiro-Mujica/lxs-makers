    ...
    return [
        {
            ...
            "imagenes":     [{"id": i.id, "url": i.url, "orden": i.orden} for i in p.imagenes],
        }
        for p in productos
    ]
...
def crear_producto(vendedor_id: str, datos: ProductoCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(nuevo)
    logger.info(f"Producto creado: {nombre} por vendedor {vendedor_id}")
...
def editar_producto(producto_id: str, datos: ProductoUpdate, db: Session = Depends(get_db)):
    ...
    if datos.nombre is not None:
        producto.nombre = bleach.clean(datos.nombre.strip())
    if datos.precio is not None:
        producto.precio = datos.precio
    if datos.descripcion is not None:
        producto.descripcion = bleach.clean(datos.descripcion.strip())
    if datos.estado is not None:
        producto.estado = datos.estado
    if datos.orden_visual is not None:
        producto.orden_visual = datos.orden_visual

    db.commit()
    db.refresh(producto)
    logger.info(f"Producto editado: {producto_id}")
