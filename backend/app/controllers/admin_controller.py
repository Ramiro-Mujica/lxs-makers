...
def aprobar_vendedor(vendedor_id: str, db: Session = Depends(get_db)):
    ...
    # Generar código de catálogo único al momento de aprobación
...
def ajustar_limite_tableros(
    ...
    logger.info(f"Límite de tableros actualizado: {vendedor.email} — límite: {limite}")
...
def estadisticas_globales(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "vendedores_pendientes":  vendedores_pendientes,
    }